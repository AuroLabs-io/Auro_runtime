...
`auro-runtime` is a governed execution boundary that sits between an untrusted model and the tools it wants to call. The model can never reach a tool directly. It first proposes a tool name, arguments, and a credential alias. Once requested, the runtime decides whether that call may go through and logs the answer either way.

The design places zero trust in the model. Directives name the tools a run may use, and a call to anything outside that list is refused. Arguments are schema-validated and policy guards evaluate each call. An empty scope is a refusal. If a security argument is *missing* rather than set, the call is refused instead of quietly skipping that boundary.

A refused call is stopped before it can take effect. First at the wall for anything outside the directive's scope, and again inside the tool for a protected path or a destination the egress guard declines. Either way the refusal is returned to the caller as a failed result rather than ending the run, and the verdict is appended to the audit log.
...
Every gate runs before anything leaves the machine. The record must assert `release_complete` and `publication_candidate`; every file it names must be present, the right size, and match its recorded SHA-256; every filename it carries must be an immediate member of the evidence directory rather than a path out of it, with no path or stream separator in it; nothing in that directory may be a symbolic link, and no named artifact may be reachable under a second name as a hard link; and the directory must contain **nothing** but the record and the files it names, since anything travelling beside the tested artifacts is something nobody tested. That last rule is deliberately strict — a stray build log refuses the publish, and the remedy is to move it out. Any failure refuses the upload, and rebuilding is not a recovery this command offers — go back and produce a record for what you actually want to publish.
